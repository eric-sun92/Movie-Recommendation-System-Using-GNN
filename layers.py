...
def _init_input_modules(g, ntype, textset, hidden_dims):
    ...
    if textset is not None:
        # Access the 'stoi' and 'vocab_vectors' directly from 'textset'
        stoi = textset.stoi
        vocab_vectors = textset.vocab_vectors
        input_dims = vocab_vectors.shape[1]
        pad_token_idx = stoi['<pad>'] 

        # Initialize BagOfWordsPretrained for text features
        module_dict['text_feature'] = BagOfWordsPretrained(stoi, vocab_vectors, pad_token_idx, hidden_dims)

    return module_dict

class BagOfWordsPretrained(nn.Module):
    def __init__(self, stoi, vocab_vectors, pad_token_idx, hidden_dims):
        super().__init__()

        input_dims = vocab_vectors.shape[1]
        self.emb = nn.Embedding(len(stoi), input_dims, padding_idx=pad_token_idx)
